...
public class NewtonLinearization extends Method1D {
    ...
    private final double lambdaDivRCdz2;
    ...
    public NewtonLinearization(Model model) {
        ...
        invDt = 1 / model.getDt();
        lambdaDivRCdz2 = model.getLambda() / (model.getR() * model.getC() * model.getDx() * model.getDx());
    ...
    public Layer1D nextLayer(Layer1D... previousLayers) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            cur[i] = -invDt - 2 * lambdaDivRCdz2 + ultimateFactor * eDivR / Math.pow(prevIterationTemperature[i], 2);
            next[i] = lambdaDivRCdz2;
        ...
        /*double dx2DivDt = model.getDx() * model.getDx() / model.getDt();
        double dx2OnK = model.getDx() * model.getDx() * model.getK();
        double EOnR = model.getE() / model.getR();
        double lambdaDivCDivP = model.getLambda() / model.getP() / model.getC();
        double dx2OnKOnQDivC = dx2OnK * model.getQ() / model.getC();
